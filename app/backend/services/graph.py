...
async def run_graph_async(graph, portfolio, tickers, start_date, end_date, model_name, model_provider, request=None, language=None):
    """Async wrapper for run_graph to work with asyncio."""
    loop = asyncio.get_running_loop()
    result = await loop.run_in_executor(None, lambda: run_graph(graph, portfolio, tickers, start_date, end_date, model_name, model_provider, request, language=language))
    return result
